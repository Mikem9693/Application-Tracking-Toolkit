...
Clicking the above link creates your own private copy in your own Google Drive. It is entirely yours from that point: your data never touches the original, and nobody else can see it. You will need to be signed into a Google account for the copy to save.

The copy opens with sample data already in it so you can see how everything works. Delete the example rows (Rows 2-7) when you are ready to start logging your own applications.
...
A metrics layer. A totals tab rolls up your whole search: applications sent, cover letters written, sift pass rate, interviews reached, offers, hit rate, rejection rate, applications per day, and a breakdown by application source.
A dashboard. The metrics feed a visual dashboard so you can see your funnel and trends at a glance rather than reading numbers off a grid.
Setup
Make your copy using the link above.
Explore the sample data. The tracker comes populated so you can see the pipeline, the totals, and the dashboard working together.
Run the automation once to authorise it. Open the Extensions menu, then Apps Script, and run either function. The first time, Google will ask you to authorise the script. This is standard for any Apps Script and it only ever runs inside your own copy of the sheet. You can review the script before running it using the "View Apps Script file" button on the copy screen.
Clear the sample rows and start logging your own applications.
...
dashboard-spec.md — the layout and contents of the dashboard, if you want to rebuild or extend it.
...
This pairs with my job application automation repository. That workflow finds and triages roles and drafts applications; this tracker records and reports on what happens to them. Together they run the full search as one system: sourcing and triage at the front, pipeline and reporting at the back.
